...
function MapInner({ experts, countryCode, center, zoom = 7, onExpertClick, isDark }: MapInnerProps) {
  ...
  useEffect(() => {
    ...
    const infoBg    = isDark ? '#1e1e30' : '#ffffff';
    const infoBorder = isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)';
    const nameColor  = isDark ? '#fff' : '#0f172a';
    const descColor  = isDark ? '#94a3b8' : '#64748b';
    const btnTextColor = isDark ? '#1a1a2e' : '#ffffff';
    ...
    experts.forEach(expert => {
      ...
      const infoContent = `
        <div style="background:${infoBg};border:1px solid ${infoBorder};border-radius:12px;padding:12px;min-width:190px;max-width:230px;font-family:system-ui,sans-serif;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            ...
            <div style="flex:1;min-width:0;">
              <p style="color:${nameColor};font-weight:600;font-size:13px;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${expert.name}</p>
              ${catNames ? `<p style="color:${color};font-size:11px;margin:0;">${catNames}</p>` : ''}
            </div>
          </div>
          ${expert.shortDesc ? `<p style="color:${descColor};font-size:11px;margin:0 0 8px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${expert.shortDesc}</p>` : ''}
          ...
          <a href="/${countryCode}/expert/${expert.profileLink}"
             style="display:block;text-align:center;background:#f97316;color:${btnTextColor};font-weight:700;font-size:12px;padding:6px 12px;border-radius:8px;text-decoration:none;">
            View Profile →
          </a>
        </div>
      `;
  ...
  }, [experts, countryCode, center, onExpertClick, isDark]);
...
function MapPlaceholder({ status }: { status: Status }) {
  return (
    <div className="w-full h-full flex items-center justify-center bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/8">
      {status === Status.LOADING ? (
        ...
        <div className="text-center text-slate-400 dark:text-slate-500 text-sm">
          ...
          Map failed to load.
          <span className="text-xs block mt-1 text-slate-400 dark:text-slate-600">Check NEXT_PUBLIC_GOOGLE_MAPS_API_KEY</span>
        </div>
      )}
    </div>
  );
...
export default function ExpertMap({ experts, countryCode, center, zoom, className = '', height }: ExpertMapProps) {
    ...
    return (
      <div className={`flex items-center justify-center bg-slate-100/60 dark:bg-slate-900/60 rounded-2xl border border-dashed border-slate-200 dark:border-white/10 ${className}`} style={containerStyle}>
        <div className="text-center text-slate-400 dark:text-slate-500 text-sm py-12">
          ...
          <p className="font-medium text-slate-500 dark:text-slate-400 mb-1">Map not configured</p>
          <p className="text-xs text-slate-400 dark:text-slate-600">Add NEXT_PUBLIC_GOOGLE_MAPS_API_KEY to .env.local</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden border border-slate-200 dark:border-white/8 ${className}`} style={containerStyle}>
      ...
      {/* Count badge */}
      <div className="absolute top-3 right-3 flex items-center gap-2 bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 backdrop-blur-sm pointer-events-none shadow-sm">
        <span>{withCoords.length} on map</span>
        {featuredCount > 0 && <span className="text-orange-500 dark:text-orange-400">• {featuredCount} featured</span>}
      </div>

      {/* Selected mini-card */}
      {selected && (
        <div className="absolute bottom-12 left-3 right-3 sm:left-auto sm:right-3 sm:w-64 bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-white/10 rounded-xl p-3 backdrop-blur-sm z-10 shadow-md">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-orange-500/20 flex items-center justify-center text-sm font-bold text-orange-500 dark:text-orange-400 shrink-0">
              {selected.name.slice(0, 2).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-slate-900 dark:text-white font-semibold text-xs truncate">{selected.name}</p>
              <p className="text-slate-400 dark:text-slate-500 text-xs truncate">{selected.categories?.[0]?.name || ''}</p>
            </div>
            <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-700 dark:text-slate-600 dark:hover:text-white text-xs ml-1 shrink-0">✕</button>
          </div>
        </div>
      )}
    </div>
