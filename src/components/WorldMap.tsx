...
const LIGHT_MAP_STYLE: google.maps.MapTypeStyle[] = [
  ...
  { featureType: 'poi',       stylers: [{ visibility: 'off' }] },
  { featureType: 'transit',   stylers: [{ visibility: 'off' }] },
  { featureType: 'administrative.country', elementType: 'geometry.stroke', stylers: [{ color: '#cbd5e1' }, { weight: 1 }] },
...
function WorldMapInner({ countries, isDark }: WorldMapInnerProps) {
  ...
  useEffect(() => {
    ...
    const infoBg      = isDark ? '#0f172a' : '#ffffff';
    const infoBorder  = isDark ? 'rgba(249,115,22,0.3)' : 'rgba(0,0,0,0.1)';
    const nameColor   = isDark ? '#fff' : '#0f172a';
    const countColor  = isDark ? '#f97316' : '#ea580c';
    ...
    countries.forEach(country => {
      ...
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="60" height="70" viewBox="0 0 60 70">
        ...
        <path d="M30 2C18.954 2 10 10.954 10 22c0 14.667 20 46 20 46S50 36.667 50 22C50 10.954 41.046 2 30 2z"
              fill="${hasExperts ? '#f97316' : (isDark ? '#374151' : '#cbd5e1')}"
              stroke="${hasExperts ? '#fed7aa' : (isDark ? '#4b5563' : '#94a3b8')}" stroke-width="1.5"/>
        <circle cx="30" cy="22" r="14" fill="${hasExperts ? (isDark ? '#1a0a00' : '#fff7ed') : (isDark ? '#1e293b' : '#f1f5f9')}"/>
      ...
      const infoContent = `
        <div style="background:${infoBg};border:1px solid ${infoBorder};border-radius:12px;padding:14px 16px;min-width:160px;font-family:system-ui,sans-serif;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
            <span style="font-size:22px;line-height:1;">${country.flagEmoji || '🌍'}</span>
            <div>
              <p style="color:${nameColor};font-weight:700;font-size:14px;margin:0;">${country.name}</p>
              ${country.expertCount ? `<p style="color:${countColor};font-size:11px;margin:0;">${country.expertCount} expert${country.expertCount !== 1 ? 's' : ''}</p>` : ''}
            </div>
          </div>
          <a href="/${country.code}" style="display:block;text-align:center;background:#f97316;color:${btnTextColor};font-weight:700;font-size:12px;padding:6px 12px;border-radius:8px;text-decoration:none;margin-top:4px;">
            Browse Experts →
          </a>
        </div>`;
  ...
  }, [mapReady, countries, isDark]); // eslint-disable-line react-hooks/exhaustive-deps
...
function WorldMapPlaceholder({ status }: { status: Status }) {
  return (
    <div className="w-full h-full flex items-center justify-center bg-slate-100 dark:bg-slate-900/80">
...
export default function WorldMap({ countries, className = '' }: WorldMapProps) {
  ...
  if (!apiKey) {
    return (
      <div className={`flex items-center justify-center bg-slate-100/60 dark:bg-slate-900/60 rounded-2xl border border-dashed border-slate-200 dark:border-white/10 ${className}`}>
        <div className="text-center text-slate-400 dark:text-slate-500 text-sm py-12">
          ...
          <p className="font-medium text-slate-500 dark:text-slate-400 mb-1">World map not configured</p>
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden border border-slate-200 dark:border-white/8 ${className}`}>
      <Wrapper apiKey={apiKey} render={(status) => <WorldMapPlaceholder status={status} />}>
        <WorldMapInner countries={countries} isDark={isDark} />
      </Wrapper>
      <div className="absolute bottom-3 left-3 bg-white/90 dark:bg-slate-900/85 border border-slate-200 dark:border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 pointer-events-none backdrop-blur-sm shadow-sm">
        Click a pin to explore
      </div>
      <div className="absolute top-3 right-3 bg-white/90 dark:bg-slate-900/85 border border-slate-200 dark:border-white/10 rounded-xl px-3 py-1.5 text-xs text-orange-500 dark:text-orange-400 pointer-events-none backdrop-blur-sm font-medium shadow-sm">
