        ...
        {expertCompletedWork.length > 0 && (
          <section className="max-w-6xl mx-auto px-6 pb-10">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Completed Work</h2>
              <Link href="/completed-work" className="text-sm text-orange-500 hover:text-orange-400 font-semibold transition-colors">
                View all →
              </Link>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {expertCompletedWork.map((item) => (
                <div key={item.id} className="rounded-2xl bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-white/8 overflow-hidden hover:border-orange-200 dark:hover:border-orange-500/30 transition-all shadow-sm dark:shadow-none flex flex-col">
                  {item.imageUrl ? (
