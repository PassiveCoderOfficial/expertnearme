...
export async function GET() {
  ...
  const items = await prisma.expertSkill.findMany({ where: { expertId: expert.id } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.expertSkill.create({ data: { expertId: expert.id, name } });
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.expertSkill.deleteMany({ where: { id, expertId: expert.id } });
